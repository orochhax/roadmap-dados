# Modelagem dimensional: fatos, dimensões, grão e SCD + Performance SQL: EXPLAIN, índices, partições e custo

**Data de estudo:** 21/09/2026  
**Carga planejada:** 4 a 5 horas

## Como estudar

Você pode escolher vídeo, documentação, site ou IA. Pesquise os nomes abaixo, faça uma primeira tentativa sem solução pronta e guarde evidência executável.

## Assuntos para pesquisar

### Bloco 1 — Modelagem dimensional: fatos, dimensões, grão e SCD

Pesquise exatamente:

- `tabela fato e dimensão`
- `granularidade de fatos`
- `star schema`
- `SCD tipo 1 e tipo 2`

Depois siga o [guia e os enunciados deste bloco](<modulos/01-n02/README.md>). Tente os exercícios antes de procurar uma implementação completa.

### Bloco 2 — Performance SQL: EXPLAIN, índices, partições e custo

Pesquise exatamente:

- `SQL EXPLAIN`
- `índices compostos`
- `partition pruning`
- `query cost`

Depois siga o [guia e os enunciados deste bloco](<modulos/02-n03/README.md>). Tente os exercícios antes de procurar uma implementação completa.

## Integração

Explique com suas palavras como os blocos se conectam em um fluxo de dados ou decisão. Execute um caso comum e um caso de borda de cada bloco e registre comandos, saídas e dúvidas nas evidências.

## Concluído quando

- [ ] Estudei todos os assuntos e concluí os enunciados dos blocos sem copiar uma solução completa.
- [ ] Executei os artefatos, testei casos de borda e registrei resultados verificáveis.
- [ ] Expliquei a conexão entre os blocos, a decisão tomada e pelo menos uma limitação concreta.
