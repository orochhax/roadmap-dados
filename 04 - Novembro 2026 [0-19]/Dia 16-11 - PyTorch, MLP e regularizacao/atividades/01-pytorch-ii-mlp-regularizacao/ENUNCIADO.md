...
O modelo linear da atividade anterior de PyTorch criou uma referência. A equipe quer saber se uma MLP justifica maior complexidade e custo. Você deverá comparar configurações sob o mesmo corte temporal, rastrear cada experimento no MLflow e recomendar um modelo somente se o ganho for estável.

## Entradas

Reutilize exatamente o dataset, as features e as divisões temporais da atividade anterior de PyTorch. Crie uma identificação de versão dos dados. O conjunto de teste continua intocado até a escolha final.
